...
class UniformQuantizer(ScalarQuantizer):
    ...
    def __init__(self, num_levels, input_peak=1.0, choice="mid-riser"):
        ...
        Examples:
            ...
            >>> quantizer = komm1.UniformQuantizer(num_levels=8)
            >>> quantizer.levels
            ...
            >>> quantizer = komm1.UniformQuantizer(num_levels=4, input_peak=1.0, choice='unsigned')
            >>> quantizer.levels
            array([0.  , 0.25, 0.5 , 0.75])
            >>> quantizer.thresholds
            array([0.125, 0.375, 0.625])

            >>> quantizer = komm1.UniformQuantizer(num_levels=4, input_peak=1.0, choice='mid-riser')
            >>> quantizer.levels
            array([-0.75, -0.25,  0.25,  0.75])
            >>> quantizer.thresholds
            array([-0.5,  0. ,  0.5])

            >>> quantizer = komm1.UniformQuantizer(num_levels=4, input_peak=1.0, choice='mid-tread')
            >>> quantizer.levels
            array([-1. , -0.5,  0. ,  0.5])
            >>> quantizer.thresholds
