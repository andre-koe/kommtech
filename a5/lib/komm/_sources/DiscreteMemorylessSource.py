...
class DiscreteMemorylessSource:
    ...
    Examples:
        ...
        >>> np.random.seed(42)
        >>> dms = komm1.DiscreteMemorylessSource([0.5, 0.4, 0.1])
        >>> dms(10)
    ...
    def entropy(self, base=2.0):
        r"""
        Returns the source entropy $\mathrm{H}(X)$. See [`komm1.entropy`](/ref/entropy) for more details.

        Parameters:

            base (Optional[float | str]): See [`komm1.entropy`](/ref/entropy). The default value is $2.0$.

        Examples:

            >>> dms = komm1.DiscreteMemorylessSource([1/2, 1/4, 1/8, 1/8])
            >>> dms.entropy()
            1.75
            >>> dms.entropy(base=4)
            0.875
